...
std::pair<Tensor, Tensor> pack_mixed_precision_cuda(Tensor data,
  ...
  Tensor packed = torch::empty({(total_bits + 31) / 32,}, options);
  ...
  compute_scale_mixed_precision_kernel<<<blocks, threads>>>(
...
__global__ void compute_scale_single_precision_kernel(int32_t bits,
  ...
  if (id < N * num_groups) {
    scale[id] = ((float)((1 << bits) - 1)) / (max[id] - min[id] + 2e-6);
  }
}

// Pack float32 data into int8 bit stream
template<bool boundary_check>
...
std::pair<Tensor, Tensor> pack_single_precision_cuda(Tensor data,
  ...
  compute_scale_single_precision_kernel<<<blocks, threads>>>(
    bits, min.data_ptr<float>(), max.data_ptr<float>(),
    scale.data_ptr<float>(), N, num_groups);
  ...
  if (N % work_per_thread == 0) {
    pack_single_precision_kernel<false><<<block_dim, thread_dim>>>(
      bits,
      data.data_ptr<float>(),
      scale.data_ptr<float>(), min.data_ptr<float>(),
    ...
    pack_single_precision_kernel<true><<<block_dim, thread_dim>>>(
      bits,
      data.data_ptr<float>(),
      scale.data_ptr<float>(), min.data_ptr<float>(),
      packed.data_ptr<int8_t>(),
...
Tensor unpack_single_precision_cuda(Tensor data,
  ...
  if (N % work_per_thread == 0) {
    unpack_single_precision_kernel<false><<<block_dim, thread_dim>>>(
      bits,
    ...
    unpack_single_precision_kernel<true><<<block_dim, thread_dim>>>(
      bits,
      data.data_ptr<int8_t>(),
      scale.data_ptr<float>(), min.data_ptr<float>(),
      unpacked.data_ptr<float>(),
